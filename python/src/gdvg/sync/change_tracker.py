...
class ChangeTracker:
    ...
    async def track_content_changes(
        ...
        async with create_tmdb_client() as tmdb:
            ...
            # Changes API returns paginated results
            page = 1
    ...
    def match_and_queue_content(
        ...
        # Get all existing content IDs from our DB
        existing_content = get_all_content_tmdb_ids()
        
        if existing_content.empty:
            logger.info("No existing content in database")
            return 0
        
        # Filter to specific content type
        type_content = existing_content[
            existing_content["content_type"] == content_type
        ]
        
        existing_ids = set(type_content["tmdb_id"].tolist())
        
        # Match changed IDs against our DB
        matched_ids = [id for id in changed_ids if id in existing_ids]
        ...
        bulk_add_to_enrichment_queue(queue_items, queue_type="content")
        
        # Update stats
        if content_type == "movie":
            self.stats["movies_queued"] = len(matched_ids)
        else:
            self.stats["tv_queued"] = len(matched_ids)
    ...
    def match_and_queue_people(
        ...
        # Get all existing people IDs from our DB
        existing_people = get_all_people_tmdb_ids()
        
        if existing_people.empty:
            logger.info("No existing people in database")
            return 0
        
        existing_ids = set(existing_people["tmdb_id"].tolist())
        ...
        bulk_add_to_enrichment_queue(queue_items, queue_type="people")
        
        self.stats["people_queued"] = len(matched_ids)
